...
function getNLPIntent(text, callback) {
    fetch("/api/nlp_intent/", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    .then(res => res.json())
    .then(data => {
        console.log("🧠 NLP Intent Result:", data);
        callback(data);
    })
    .catch((err) => {
        console.error("❌ NLP intent fallback failed:", err);
        callback({ intent: "unknown", name: "", quantity: "1" });
    });
}
